...
import { createUnMarkEditableAction } from '../state/contactsActions';
import { createAddActionThunk, createUpdateActionThunk } from '../state/contactActionThunks';
...
const ContactForm = ({ contact }) => {
    ...
    const formSubmitted = event => {
        ...
        let action = isEditing ?
            createUpdateActionThunk({ id, fullName, mailId, mobile, isEditing }) :
            createAddActionThunk({ id, fullName, mailId, mobile, isEditing });
        dispatch(action);
        setFullName('');
        setMailId('');
        setMobile('');
